...
# 导入基础工具类
# BaseTool 是所有工具的基类，可以通过继承它来创建自定义工具
...
与 00.tool_InjectedToolArg_2.py 的区别：
→ 本脚本使用 BaseTool 类来定义工具（更底层的方式）
→ 之前的脚本使用 @tool 装饰器（更简洁的方式）
→ 两种方式都能实现相同的功能，但 BaseTool 提供更多自定义能力
...
class UpdateFavoritePets(BaseTool):
    ...
    什么时候使用 BaseTool？
    → 需要自定义工具的行为
    → 需要支持同步和异步执行（_run 和 _arun）
    → 需要更精细的控制
    """

    # 定义工具名称
    # 这个名称会被LLM看到，用于决定调用哪个工具
    name: str = "update_favorite_pets"

    # 定义工具描述
    # 这个描述会被LLM看到，帮助它理解工具的用途
    ...
    # Optional[Type[BaseModel]] 表示：
    # → 可以是 None（不指定schema）
    # → 也可以是一个 BaseModel 的子类
    # 这里指定为 UpdateFavoritePetsSchema
...
# 创建工具实例
# 实例化 UpdateFavoritePets 类，得到一个可用的工具对象
...
@tool(parse_docstring=True)
def list_favorite_pets(user_id: Annotated[str, InjectedToolArg]) -> list:
    """列出最喜爱的宠物。
...
# 2. 执行 _run 方法：user_to_pets["123"] = ["lizard", "dog"]
# 3. 保存成功
...
# 创建工具列表
tools = [
    update_favorite_pets,  # 更新宠物列表（使用 BaseTool 定义）
    delete_favorite_pets,  # 删除宠物列表（使用 @tool 定义）
    list_favorite_pets,  # 查询宠物列表（使用 @tool 定义）
]
# 注意：这里混合使用了两种不同的工具定义方式
# → update_favorite_pets: 通过继承 BaseTool 创建
# → delete_favorite_pets 和 list_favorite_pets: 通过 @tool 装饰器创建
# 两种方式创建的工具可以一起使用！
